...
const { getNote, note, updateNote } = useNote();
...
const getTotalFinal = () => {
  let suma = itemsSelected.value.reduce((total, elemento) => {
    if (!isNaN(parseFloat(elemento.total))) {
      return total + parseFloat(elemento.total);
    } else {
      return total;
    }
  }, 0);
...
const saveNote = async () => {
  ...
  createData()
  await updateNote(data.value, props.id);
};

// al montar el evento
onMounted(async () => {
  await getNote(props.id); console.log(note.value);
  await getCustomers(); console.log(customers.value);
  await getItems(); console.log(items.value);

  customer.value = note.value.customer_id;
  totalFinal.value = parseFloat(note.value.total);
  for (let i = 0; i < note.value.items.length; i++) {
    const item = note.value.items[i];
    itemsSelected.value.push({
      id: item.id, 
      name: item.name, 
      price: item.price, 
...
<template>
  ...
  <div class="container">
    <div class="row align-items-center mb-2">
      <div class="col-6">
        <div class="h3">Nota <small class="text-muted" style="font-size: 16px; font-weight: 500;">Editar</small></div>
      </div>
      <div class="col-6">
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="form-group">
              <label>Id:</label>
              <input type="text" class="form-control" min="0" v-model="props.id" disabled>
            </div>
